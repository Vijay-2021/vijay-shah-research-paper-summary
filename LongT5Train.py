...
save_path = f"{data_dir}/experiments/LongT5/models/output2.pt"
...
train_dataset = load_dataset("narrativeqa", split="train[:300]")
val_dataset = load_dataset("narrativeqa", split="validation[:100]") # run some tests on a smaller sample size for now
...
model = AutoModelForSeq2SeqLM.from_pretrained("google/long-t5-tglobal-base", offload_folder="offload", torch_dtype=torch.float16).half().cuda()
